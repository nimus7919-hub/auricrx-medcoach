...
export default function MedicalDocumentsScreen({ onClose, theme, S }: MedicalDocumentsScreenProps) {
  ...
  const DOCUMENT_CATEGORIES = {
    ...
    birth_certificate: {
      ...
      frontLabel: t('document'),
      backLabel: t('document')
    },
    insurance: {
  ...
  const shareToGmail = async (document: DocumentItem) => {
    try {
      triggerHaptic('light');
      setShowSharingModal(false);
      
      console.log('Sharing to Gmail:', document.name);
      ...
      if (isPDF) {
        // For PDFs, use IntentLauncher to open Gmail directly
        try {
          const fileInfo = await FileSystem.getInfoAsync(document.uri);
          if (fileInfo.exists) {
            console.log('PDF file exists, opening Gmail directly...');
            ...
            // Try to open Gmail directly using expo-sharing
            try {
              console.log('📧 Attempting to open Gmail with expo-sharing...');
              console.log('📧 File URI:', gmailUri);
              console.log('📧 Doctor email:', doctor.email);
              
              // Use expo-sharing with Gmail intent
              const isAvailable = await Sharing.isAvailableAsync();
              if (isAvailable) {
                await Sharing.shareAsync(gmailUri, {
                  mimeType: 'application/pdf',
                  dialogTitle: `Share Medical Document with Dr. ${doctor.name}`,
                });
                console.log('✅ Gmail opened successfully with expo-sharing');
              } else {
              ...
              // Fallback: Use Share.share
              const shareResult = await Share.share({
                url: gmailUri,
                title: `Medical Document: ${document.name}`,
                message: `Hi Dr. ${doctor.name},\n\nPlease find attached: ${document.name}\n\nBest regards,\n[Your Name]`,
              });
              
              if (shareResult.action === Share.sharedAction) {
                console.log('✅ Gmail opened with Share API');
              } else {
          ...
          console.error('Failed to open Gmail directly:', error);
          Alert.alert('❌ Error', 'Failed to open Gmail directly. Please try using the general sharing option.');
        }
      } else {
        ...
        try {
          ...
          const shareResult = await Share.share({
            url: shareableUri,
            title: document.name,
            message: `Medical Document: ${document.name}`,
          });
      ...
      Alert.alert('❌ ' + t('error'), 'Failed to share to Gmail');
    }
  };
  ...
  const shareToAllApps = async (document: DocumentItem) => {
    try {
      ...
      console.log('Sharing to all apps:', document.name);
      ...
      if (isPDF) {
        // For PDFs, use expo-sharing for better compatibility
        try {
          const fileInfo = await FileSystem.getInfoAsync(document.uri);
          if (fileInfo.exists) {
            console.log('PDF file exists, sharing with expo-sharing...');
            ...
            console.log('PDF shared successfully with expo-sharing');
          } else {
            Alert.alert('❌ ' + t('error'), 'Document file not found');
          }
        } catch (shareError) {
          console.error('expo-sharing failed:', shareError);
          
          // Fallback to React Native Share API
          try {
            console.log('Falling back to React Native Share API...');
            await Share.share({
              url: document.uri,
              title: document.name,
            });
            console.log('PDF shared successfully with React Native Share API');
          } catch (fallbackError) {
            console.error('React Native Share API also failed:', fallbackError);
            Alert.alert('❌ ' + t('error'), 'Failed to share PDF. Please try a different app.');
          }
        }
      } else {
        // For images, convert to shareable format and include the actual image
        try {
          console.log('🔍 DEBUGGING IMAGE SHARING...');
          console.log('🔍 Document name:', document.name);
          console.log('🔍 Document URI:', document.uri);
          console.log('🔍 Document type:', document.type);
          
          // Convert to shareable URI first
          console.log('🔄 Converting URI to shareable format...');
          const shareableUri = await convertToShareableUri(document.uri);
          console.log('🔄 Shareable URI:', shareableUri);
          
          // Check if the converted file exists
          const fileInfo = await FileSystem.getInfoAsync(shareableUri);
          console.log('🔍 Converted file exists:', fileInfo.exists);
          console.log('🔍 Converted file size:', fileInfo.size);
          
          if (!fileInfo.exists) {
            Alert.alert('❌ Error', 'Could not create shareable file');
            return;
          }
          
          // Try sharing with the actual file URI
          console.log('📤 Trying Share.share with actual file URI...');
          console.log('📤 Shareable URI being shared:', shareableUri);
          console.log('📤 Document name:', document.name);
          
          const shareResult = await Share.share({
            message: `AuricRX Medical Document: ${document.name}`,
            url: shareableUri, // This is the key - include the actual file
            title: document.name,
          });
          
          console.log('📤 Share result:', shareResult);
          console.log('✅ Share with file URI successful');
          
        } catch (shareError) {
          console.error('❌ Share with file URI failed:', shareError);
          
          // Fallback to expo-sharing
          try {
            console.log('📤 Trying expo-sharing as fallback...');
            await Sharing.shareAsync(shareableUri, {
              mimeType: 'image/jpeg',
              dialogTitle: `Share ${document.name}`,
            });
            ...
            console.error('❌ Both methods failed:', expoError);
            Alert.alert('❌ Share Failed', 'Unable to share image. Error: ' + expoError.message);
          }
        }
      }
    } catch (error) {
  ...
  const viewDocument = async (document: DocumentItem) => {
    try {
      ...
      if (isPDFFile(document)) {
        ...
        // Check if this is an ID document that should use dual viewer
        if (isIDDocument(document)) {
          console.log('Opening ID document in dual viewer...');
          const pair = findIDPair(document);
          ...
          setBackImageDataUri(null);
          setShowDualIDViewer(true);
          
          // Convert both images to data URIs
          try {
            const frontDataUri = pair && pair.front ? await convertImageToDataUri(pair.front.uri) : null;
            const backDataUri = pair && pair.back ? await convertImageToDataUri(pair.back.uri) : null;
            
            setImageDataUri(frontDataUri);
            setBackImageDataUri(backDataUri);
            setImageLoading(false);
            setImageError(false);
            
            console.log('✅ ID pair loaded - Front:', !!frontDataUri, 'Back:', !!backDataUri);
        } catch (error) {
            console.error('❌ Error loading ID pair:', error);
            setImageLoading(false);
            setImageError(true);
          }
        } else {
              ...
              {DOCUMENT_CATEGORIES[selectedCategory].supportsDualSides ? (
                <>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      takePhoto(selectedCategory as DocumentCategory, 'front');
                    }}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setShowUploadModal(false);
                      takePhoto(selectedCategory as DocumentCategory, 'back');
                    }}
                  >
                    <DynamicText type="card" style={dynamicStyles.modalButtonText}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setShowUploadModal(false);
                      uploadDocument(selectedCategory as DocumentCategory, 'front');
                    }}
                  >
                    <DynamicText type="card" style={dynamicStyles.modalButtonText}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setShowUploadModal(false);
                      uploadDocument(selectedCategory as DocumentCategory, 'back');
                    }}
                  >
                    <DynamicText type="card" style={dynamicStyles.modalButtonText}>
                  ...
                  {selectedCategory !== 'photo_id' && (
                    <>
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          setShowUploadModal(false);
                          uploadPDF(selectedCategory as DocumentCategory, 'front');
                        }}
                      >
                        <DynamicText type="card" style={dynamicStyles.modalButtonText}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          setShowUploadModal(false);
                          uploadPDF(selectedCategory as DocumentCategory, 'back');
                        }}
                      >
                        <DynamicText type="card" style={dynamicStyles.modalButtonText}>
                ...
                <>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setShowUploadModal(false);
                      takePhoto(selectedCategory as DocumentCategory, 'single');
                    }}
                  >
                    <DynamicText type="card" style={dynamicStyles.modalButtonText}>
                      📷 {t('takePhoto')}
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setShowUploadModal(false);
                      uploadDocument(selectedCategory as DocumentCategory, 'single');
                    }}
                  >
                    <DynamicText type="card" style={dynamicStyles.modalButtonText}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setShowUploadModal(false);
                      uploadPDF(selectedCategory as DocumentCategory, 'single');
                    }}
                  >
                    <DynamicText type="card" style={dynamicStyles.modalButtonText}>
                      📄 Upload PDF Document
