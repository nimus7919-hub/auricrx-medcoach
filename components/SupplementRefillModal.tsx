...
import { Modal, View, Text, Pressable, Animated, Easing, FlatList, ActivityIndicator, Image, Alert } from "react-native";
...
interface SupplementInfo { name: string; brand?: string; dosage?: string; lastRefill?: string }
interface Props { visible: boolean; onClose: () => void; supplement: SupplementInfo; strings: any; lang: string }

export default function SupplementRefillModal({ visible, onClose, supplement, strings, lang }: Props) {
  ...
  async function load() {
    try {
      ...
      const near = await findNearbySupplementStores(coords.latitude, coords.longitude, supplement.name, lang);
      const { prices, meta } = await getSupplementPrices(near, supplement.name, { currency: determinedCurrency.toUpperCase() });
      setResults(prices);
        ...
        <Animated.View style={{
          ...
          <View style={{ gap: spacing.sm }}>
            ...
            <Text style={{ color: colors.sub, fontSize: 14 }}>
              {supplement.name} • {supplement.dosage || supplement.brand}{supplement.lastRefill ? ` • ${(strings.lastRefill||'Last refill')}: ${supplement.lastRefill}` : ""}
            </Text>
            {mockMode && (
              <View style={{ backgroundColor: '#fbbf24', paddingHorizontal: spacing.md, paddingVertical: 6, borderRadius: radius.xl }}>
                <Text style={{ color:'#000', fontSize:12, fontWeight:'600' }}>Mock data (add GOOGLE_PLACES_API_KEY for live stores)</Text>
              </View>
            )}
            {coordsUsed && (
